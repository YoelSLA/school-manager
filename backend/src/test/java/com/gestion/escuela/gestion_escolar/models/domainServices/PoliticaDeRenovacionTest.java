...
class PoliticaDeRenovacionTest {
  ...
  class ValidarRenovarProvisionalManualTest {
    ...
    void debeLanzarExcepcionSiNuevoPeriodoComienzaAntesDelDiaPermitido() {
      ...
      when(periodoAnterior.getFechaHasta()).thenReturn(LocalDate.of(2025, 2, 28));

      Periodo nuevoPeriodo = mock(Periodo.class);

      when(nuevoPeriodo.getFechaDesde()).thenReturn(LocalDate.of(2025, 2, 28));
    ...
    void debeValidarCorrectamenteUnaRenovacionManualValida() {
      ...
      when(periodoAnterior.getFechaHasta()).thenReturn(LocalDate.of(2024, 2, 29));

      Periodo nuevoPeriodo = mock(Periodo.class);

      when(nuevoPeriodo.getFechaDesde()).thenReturn(LocalDate.of(2025, 3, 1));

      when(nuevoPeriodo.getFechaHasta()).thenReturn(LocalDate.of(2025, 12, 31));
  ...
  class ValidarRenovarProvisionalDesdeMarzoTest {
    ...
    void debeLanzarExcepcionSiFechaHastaEsNull() {
      ...
      assertThatThrownBy(
              () -> PoliticaDeRenovacion.validarRenovarProvisionalDesdeMarzo(asignacion, null))
          .isInstanceOf(CampoObligatorioException.class)
    ...
    void noDebeLanzarExcepcionConParametrosValidos() {
      ...
      assertThatCode(
              () ->
                  PoliticaDeRenovacion.validarRenovarProvisionalDesdeMarzo(
                      asignacion, LocalDate.of(2025, 12, 31)))
          .doesNotThrowAnyException();
    }
  }
  ...
  class ValidarReglaCicloLectivoTest {
    ...
    void debeLanzarExcepcionSiNoIniciaElPrimeroDeMarzo() {
      ...
      when(periodo.getFechaDesde()).thenReturn(LocalDate.of(2025, 3, 2));
    ...
    @Test
    @DisplayName(
        "debe lanzar excepción si la fecha hasta no es anterior al 1 de marzo del año siguiente")
    void debeLanzarExcepcionSiFechaHastaNoEsAnteriorAlPrimeroDeMarzoSiguiente() {
      ...
      when(periodo.getFechaHasta()).thenReturn(LocalDate.of(2026, 3, 1));
    ...
    void debeValidarCorrectamentePeriodoValido() {
      ...
      assertThatCode(() -> PoliticaDeRenovacion.validarReglaCicloLectivo(periodo))
          .doesNotThrowAnyException();
    }
  }
}
