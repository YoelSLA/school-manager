...
class MateriaControllerTest {
  ...
  @MockitoBean private MateriaService materiaService;
  ...
  void debeCrearMateria() throws Exception {
    ...
    when(materiaService.crear(eq(1L), any(Materia.class))).thenReturn(materia);

    MateriaCreateDTO dto = new MateriaCreateDTO("Matemática", "MAT", 4);

    mockMvc
        .perform(
            post("/api/escuelas/1/materias")
                .with(csrf())
  ...
  void debeCrearMateriasEnBatch() throws Exception {
    ...
    List<MateriaCreateDTO> dtos =
        List.of(
            new MateriaCreateDTO("Matemática", "MAT", 4), new MateriaCreateDTO("Lengua", "LEN", 3));

    mockMvc
        .perform(
            post("/api/escuelas/1/materias/batch")
                .with(csrf())
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(dtos)))
        .andExpect(status().isCreated());

    verify(materiaService)
        .crearBatch(
            eq(1L),
            argThat(
                lista ->
  ...
  void debeActualizarMateria() throws Exception {
    ...
    MateriaUpdateDTO dto = new MateriaUpdateDTO("Matemática Avanzada", "MAT2", 5);

    Materia materiaActualizada = new Materia("Matemática Avanzada", "MAT2", 5);

    ReflectionTestUtils.setField(materiaActualizada, "id", 10L);

    when(materiaService.actualizar(eq(1L), eq(10L), eq("Matemática Avanzada"), eq("MAT2"), eq(5)))
        .thenReturn(materiaActualizada);

    mockMvc
        .perform(
            put("/api/escuelas/1/materias/10")
                .with(csrf())
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(dto)))
  ...
  void debeEliminarMateria() throws Exception {
    ...
    mockMvc
        .perform(delete("/api/escuelas/1/materias/10").with(csrf()))
  ...
  void debeListarMateriasPaginadas() throws Exception {
    ...
    Page<Materia> page = new PageImpl<>(List.of(m1, m2), PageRequest.of(0, 10), 2);

    when(materiaService.listarMateriasPorEscuela(eq(1L), any(Pageable.class))).thenReturn(page);

    mockMvc
        .perform(get("/api/escuelas/1/materias").param("page", "0").param("size", "10"))
        .andExpect(status().isOk())
