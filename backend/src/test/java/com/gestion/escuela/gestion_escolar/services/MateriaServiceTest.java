...
class MateriaServiceTest extends DomainServiceFixtureTest {
  ...
  @BeforeEach
  void setUp() {
    Escuela escuelaModelo =
        new Escuela("Escuela N°65", "San Francisco Solano", "Avenida Provincial 849", "1142130357");
  ...
  class CrearMateria {
    ...
    void creaMateriaCorrectamente() {
      ...
      Materia materiaM = new Materia("Ciencias Naturales", "CNT", 4);
  ...
  class CrearBatchTest {
    ...
    void fallaSiEscuelaNoExiste() {
      ...
      assertThrows(
          RecursoNoEncontradoException.class, () -> materiaService.crearBatch(999L, materias));
    }
    ...
    void fallaSiAlgunaMateriaEsDuplicada() {
      ...
      materiaService.crear(escuelaBDD.getId(), new Materia("Química", "QUI", 4));

      List<Materia> nuevas =
          List.of(new Materia("Química", "QUI2", 6), new Materia("Biología", "BIO", 4));
    ...
    void creaTodasLasMateriasCorrectamente() {
      ...
      List<Materia> materias =
          List.of(new Materia("Historia", "HIS", 4), new Materia("Geografía", "GEO", 3));

      materiaService.crearBatch(escuelaBDD.getId(), materias);

      List<Materia> guardadas = materiaService.listarMateriasPorEscuela(escuelaBDD.getId());
  ...
  class ObtenerPorIdTest {
    ...
    void devuelveMateriaSiExiste() {
      ...
      // Arrange
      Materia creada = materiaService.crear(escuelaBDD.getId(), new Materia("Física", "FIS", 4));
    ...
    void fallaSiNoExiste() {
      ...
      assertThrows(RecursoNoEncontradoException.class, () -> materiaService.obtenerPorId(999999L));
    }
  }
  ...
  class ListarMateriasPorEscuelaTest {
    ...
    void listaOrdenadoPorNombreAsc() {
      ...
      // Arrange
      materiaService.crear(escuelaBDD.getId(), new Materia("Zoología", "ZOO", 4));

      materiaService.crear(escuelaBDD.getId(), new Materia("Biología", "BIO", 4));

      materiaService.crear(escuelaBDD.getId(), new Materia("Análisis Matemático", "ANA", 4));

      Pageable pageable = PageRequest.of(0, 10);

      // Act
      Page<Materia> pagina = materiaService.listarMateriasPorEscuela(escuelaBDD.getId(), pageable);
    ...
    void respetaLaPaginacion() {
      ...
      materiaService.crear(escuelaBDD.getId(), new Materia("A", "A", 4));

      materiaService.crear(escuelaBDD.getId(), new Materia("B", "B", 4));

      materiaService.crear(escuelaBDD.getId(), new Materia("C", "C", 4));
  ...
  class EliminarTest {
    ...
    void fallaSiEscuelaNoExiste() {
      ...
      assertThrows(RecursoNoEncontradoException.class, () -> materiaService.eliminar(999L, 1L));
    }
    ...
    void eliminaMateriaCorrectamente() {
      ...
      // Arrange
      Materia creada = materiaService.crear(escuelaBDD.getId(), new Materia("Filosofía", "FIL", 4));

      // Act
      materiaService.eliminar(escuelaBDD.getId(), creada.getId());

      // Assert
      assertThrows(
          RecursoNoEncontradoException.class, () -> materiaService.obtenerPorId(creada.getId()));
    }
    ...
    void fallaSiMateriaPerteneceAOtraEscuela() {
      ...
      Escuela otraEscuelaModelo = new Escuela("test", "test", "test", "11421303");

      Escuela otraEscuela = escuelaService.crear(otraEscuelaModelo);

      Materia materia = materiaService.crear(otraEscuela.getId(), new Materia("Arte", "ART", 4));

      assertThrows(
          RecursoNoEncontradoException.class,
          () -> materiaService.eliminar(escuelaBDD.getId(), materia.getId()));
    }
  }
  ...
  class ActualizarTest {
    ...
    void fallaSiMateriaNoExiste() {
      ...
      assertThrows(
          RecursoNoEncontradoException.class,
          () -> materiaService.actualizar(escuelaBDD.getId(), 999L, "Nuevo Nombre", "NN", 6));
    }
    ...
    void actualizaCorrectamente() {
      ...
      // Arrange
      Materia creada = materiaService.crear(escuelaBDD.getId(), new Materia("Historia", "HIS", 4));

      // Act
      Materia actualizada =
          materiaService.actualizar(
              escuelaBDD.getId(), creada.getId(), "Historia Argentina", "HISA", 6);
    ...
    void fallaSiDatosInvalidos() {
      ...
      Materia creada = materiaService.crear(escuelaBDD.getId(), new Materia("Geografía", "GEO", 4));

      assertThrows(
          CampoObligatorioException.class,
          () -> materiaService.actualizar(escuelaBDD.getId(), creada.getId(), "", "GEO", 4));
    }
  }
}
