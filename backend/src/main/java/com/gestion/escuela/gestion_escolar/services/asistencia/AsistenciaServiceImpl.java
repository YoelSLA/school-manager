...
public class AsistenciaServiceImpl implements AsistenciaService {
  ...
  public void registrarInasistencia(
    ...
    Optional<Asistencia> asistenciaExistente =
        asistenciaRepository.findByEmpleadoEducativoIdAndEscuelaIdAndFecha(
            empleadoEducativo.getId(), escuelaId, fecha);

    Asistencia asistencia;

    if (asistenciaExistente.isPresent()) {
      asistencia = asistenciaExistente.get();
      asistencia.actualizarManual(licenciaEstatutaria, observacion);
    } else {
      asistencia =
          new Asistencia(
              empleadoEducativo, fecha, EstadoAsistencia.AUSENTE, licenciaEstatutaria, observacion);
    }
  ...
  public void registrarInasistencias(
    ...
    for (LocalDate fecha : fechas) {
      registrarInasistencia(escuelaId, empleado.getId(), fecha, licenciaEstatutaria, observacion);
    }
  }

  @Override
  public List<Asistencia> obtenerAsistenciasDelMes(Long escuelaId, Long empleadoId, YearMonth mes) {

    LocalDate inicio = mes.atDay(1);
    LocalDate fin = mes.atEndOfMonth();

    return asistenciaRepository.findByEmpleadoEducativoIdAndEscuelaIdAndFechaBetween(
        empleadoId, escuelaId, inicio, fin);
  }
    ...
    Map<LocalDate, Asistencia> manualesPorFecha =
        asistenciaRepository
            .findByEmpleadoEducativoIdAndEscuelaIdAndFechaBetween(
                empleadoId, escuelaId, inicio, fin)
            .stream()
            .collect(Collectors.toMap(Asistencia::getFecha, asistencia -> asistencia));

    return empleado.diasQueDebeAsistir(mes).stream()
        .map(fecha -> empleado.estadoAsistenciaEn(fecha, manualesPorFecha))
        .toList();
  }

  @Override
  @Transactional(readOnly = true)
  public List<RolCantidad> contarEmpleadosPorRolVigente(Long escuelaId, LocalDate fecha) {

    List<RolCantidadProjection> conteos =
        asignacionRepository.contarEmpleadosPorRolVigente(escuelaId, fecha);

    Map<RolEducativo, Integer> conteo = new EnumMap<>(RolEducativo.class);

    conteos.forEach(p -> conteo.put(p.getRol(), p.getCantidad().intValue()));

    return Arrays.stream(RolEducativo.values())
        .map(rol -> new RolCantidad(rol, conteo.getOrDefault(rol, 0)))
  ...
  public Page<EmpleadoEducativo> buscarEmpleados(
    ...
    return empleadoEducativoRepository.buscarEmpleadosConRolVigente(
        escuelaId, fecha, rolesFiltro, queryFiltro, pageable);
  }

  @Override
  public void eliminarInasistencia(Long escuelaId, Long empleadoId, LocalDate fecha) {

    Asistencia asistencia =
        asistenciaRepository
            .findByEmpleadoEducativoIdAndEscuelaIdAndFecha(empleadoId, escuelaId, fecha)
  ...
  @Override
  public EmpleadoAsistenciaResumen getResumenAsistenciaEmpleado(Long empleadoId, LocalDate fecha) {

    LocalDate fechaDesde = fecha.withDayOfMonth(1);
    LocalDate fechaHasta = fecha.withDayOfMonth(fecha.lengthOfMonth());

    int faltasUltimoMes =
        (int)
            asistenciaRepository.countByEmpleadoEducativoIdAndFechaBetweenAndEstadoAsistenciaIn(
                empleadoId, fechaDesde, fechaHasta, List.of(EstadoAsistencia.AUSENTE));

    LicenciaEstatutaria licenciaMasFrecuente =
        asistenciaRepository
            .findLicenciasEstatutariasMasFrecuentesDelMes(
                empleadoId, fechaDesde, fechaHasta, PageRequest.of(0, 1))
            .stream()
            .findFirst()
            .orElse(null);

    return new EmpleadoAsistenciaResumen(
        empleadoEducativoRepository.obtenerRolesActivosEn(empleadoId, fecha),
        faltasUltimoMes,
