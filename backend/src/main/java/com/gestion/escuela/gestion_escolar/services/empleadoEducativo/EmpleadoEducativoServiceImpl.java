...
public class EmpleadoEducativoServiceImpl implements EmpleadoEducativoService {
    ...
    if (empleadoEducativoRepository.existsByCuilAndEscuelaId(empleado.getCuil(), escuelaId)) {
      throw new RecursoDuplicadoException(
          String.format(
              ...
              empleado.getCuil(), escuela.getNombre()));
    }
    ...
    if (empleadoEducativoRepository.existsByEmailAndEscuelaId(empleado.getEmail(), escuelaId)) {
      throw new RecursoDuplicadoException(
          String.format(
              ...
              empleado.getEmail(), escuela.getNombre()));
    }
  ...
  @Override
  public EmpleadoEducativo obtenerPorId(Long empleadoId) {
  ...
  @Override
  @Transactional
  public void darDeBajaDefinitiva(Long empleadoId, LocalDate fechaBaja, CausaBaja causa) {

    EmpleadoEducativo empleado = obtenerPorId(empleadoId);

    empleado.setActivo(false);

    Set<Asignacion> asignaciones = empleado.asignacionesAfectadasPorBaja(fechaBaja);

    for (Asignacion asignacion : asignaciones) {

      boolean generaVacante = asignacion.puedeGenerarVacanteDefinitiva();

      asignacion.finalizarPorBajaDefinitiva(causa, fechaBaja);

      if (generaVacante) {

        asignacionRepository
            .buscarSuplenciaActiva(asignacion.getDesignacion().getId(), fechaBaja)
            .ifPresent(s -> s.convertirseEnProvisional(fechaBaja.plusDays(1), s.getSecuencia()));
      }
    }
  }
  ...
  public Page<EmpleadoEducativo> listarPorEscuela(
      Long escuelaId, Boolean estado, Pageable pageable) {
    ...
    if (estado == null) {
      return empleadoEducativoRepository.findByEscuelaId(escuelaId, pageable);
    }

    return empleadoEducativoRepository.findByEscuelaIdAndActivo(escuelaId, estado, pageable);
  }
  ...
  @Override
  public Set<LocalDate> diasLaborablesEnPeriodo(Long escuelaId, Long empleadoId, Periodo periodo) {
  ...
  public EmpleadoEducativo actualizar(EmpleadoEducativo empleado) {
    ...
    if (empleadoEducativoRepository.existsByCuilAndEscuelaIdAndIdNot(
        empleado.getCuil(), escuelaId, empleadoId)) {
      throw new RecursoDuplicadoException(
          String.format(
              "Ya existe un empleadoEducativoBasico con CUIL '%s' en la escuela '%s'.",
              empleado.getCuil(), escuela.getNombre()));
    ...
    if (empleadoEducativoRepository.existsByEmailAndEscuelaIdAndIdNot(
        empleado.getEmail(), escuelaId, empleadoId)) {
      throw new RecursoDuplicadoException(
          String.format(
              "Ya existe un empleadoEducativoBasico con email '%s' en la escuela '%s'.",
              empleado.getEmail(), escuela.getNombre()));
