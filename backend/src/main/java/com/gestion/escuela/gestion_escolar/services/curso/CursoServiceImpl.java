...
public class CursoServiceImpl implements CursoService {
  ...
  public Curso crear(Long escuelaId, Curso curso) {
    ...
    validarNoDuplicado(
        escuelaId, curso.getAnio(), curso.getGrado(), curso.getTurno(), escuela.getNombre());
  ...
  @Override
  public Curso obtenerPorId(Long cursoId) {
  ...
  @Override
  public Page<Curso> listarCursosPorEscuela(Long escuelaId, Turno turno, Pageable pageable) {
  ...
  public Curso actualizar(Long escuelaId, Long cursoId, Integer anio, Integer grado, Turno turno) {
    ...
    if (cursoRepository.existsByEscuelaIdAndAnioAndGradoAndTurnoAndIdNot(
        escuelaId, anio, grado, turno, cursoId)) {
      throw new RecursoDuplicadoException(
          String.format(
              "Ya existe un curso %s° %s - turno %s en la escuela %s",
              anio, grado, turno, escuela.getNombre()));
    }
  ...
  private void validarNoDuplicado(
      Long escuelaId, Integer anio, Integer grado, Turno turno, String nombreEscuela) {
    if (cursoRepository.existsByEscuelaIdAndAnioAndGradoAndTurno(escuelaId, anio, grado, turno)) {
      throw new RecursoDuplicadoException(
          String.format(
              "Ya existe un curso %s° %s - turno %s en la escuela %s",
              anio, grado, turno, nombreEscuela));
    }
  }
}
