...
public class MateriaServiceImpl implements MateriaService {
  ...
  public Materia crear(Long escuelaId, Materia materia) {
    ...
    validarNombreUnico(escuelaId, materia.getNombre(), escuela.getNombre());
  ...
  @Override
  public Materia obtenerPorId(Long materiaId) {
  ...
  public Materia actualizar(
    ...
    if (materiaRepository.existsByEscuelaIdAndNombreIgnoreCaseAndIdNot(
        escuelaId, nombre, materiaId)) {
      throw new RecursoDuplicadoException(
          String.format(
              "Ya existe una materia con nombre '%s' en la escuela %s",
              nombre, materia.getEscuela().getNombre()));
    }
  ...
  private void validarNombreUnico(Long escuelaId, String nombre, String nombreEscuela) {
    if (materiaRepository.existsByNombreIgnoreCaseAndEscuelaId(nombre, escuelaId)) {
      throw new RecursoDuplicadoException(
          String.format(
              "Ya existe una materia con nombre '%s' en la escuela %s", nombre, nombreEscuela));
    }
  }
}
