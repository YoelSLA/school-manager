...
public class EscuelaEmpleadoEducativoControllerREST {
  ...
  public EmpleadoEducativoDetalleDTO crear(
      @PathVariable Long escuelaId, @Valid @RequestBody EmpleadoEducativoCreateDTO dto) {
    Escuela escuela = escuelaService.obtenerPorId(escuelaId);
    EmpleadoEducativo empleado =
        empleadoEducativoService.crear(escuelaId, EmpleadoEducativoMapper.toEntity(dto, escuela));
  ...
  public EmpleadoEducativoDetalleDTO actualizar(
    ...
    EmpleadoEducativo empleadoActual =
        empleadoEducativoService.obtenerPorEscuela(escuelaId, empleadoId);
  ...
  public void crearBatch(
      @PathVariable Long escuelaId, @RequestBody List<EmpleadoEducativoCreateDTO> empleadosDTOs) {
    Escuela escuela = escuelaService.obtenerPorId(escuelaId);
    List<EmpleadoEducativo> empleadoEducativos =
        empleadosDTOs.stream().map(dto -> EmpleadoEducativoMapper.toEntity(dto, escuela)).toList();
  ...
  public PageResponse<EmpleadoEducativoResumenDTO> listar(
    ...
    Page<EmpleadoEducativo> empleados =
        empleadoEducativoService.listarPorEscuela(escuelaId, estadoFiltro, limitedPageable);

    return PageMapper.toPageResponse(
        empleados,
        e ->
            EmpleadoEducativoMapper.toResumen(
                e, empleadoEducativoService.obtenerRolesEducativos(e.getId())));
  }

  @GetMapping(params = "search")
  public List<EmpleadoEducativoConRolesDTO> buscar(
      @PathVariable Long escuelaId, @RequestParam String search) {
    return empleadoEducativoService.buscarPorEscuela(escuelaId, search).stream()
        .map(
            empleado -> {
              Set<RolEducativo> roles =
                  empleadoEducativoService.obtenerRolesEducativos(empleado.getId());
