...
export default function DesignacionAdministrativaCreatePage() {
  ...
  const {
    ...
    defaultValues: {
      ...
      franjasHorarias: [{ dia: "LUNES", desde: "08:00", hasta: "12:00" }],
    },
  });
        ...
        <div className="franjas">
          ...
          {fields.map((field, index) => (
            <div key={field.id} className="franja-row">
              ...
              <input
                type="time"
                {...register(`franjasHorarias.${index}.desde`)}
              />

              <input
                type="time"
                {...register(`franjasHorarias.${index}.hasta`)}
              />
              ...
              {errors.franjasHorarias?.[index] && (
                <span className="error">
                  {errors.franjasHorarias[index]?.hasta?.message}
                </span>
              )}
            </div>
          ))}
