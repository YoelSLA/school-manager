...
import type { SortDirection, SortState } from "@/shared/types";
...
export default function EmpleadoSortDropdown({ value, onChange }: Props) {
	...
	const _hasActiveSort = value.nombre || value.apellido || value.fechaDeIngreso;

	const handleChange = (field: keyof SortState, direction: SortDirection) => {
		onChange({
			...value,
			[field]: direction,
		});
			...
			{open && (
				<div className={styles.sort__menu}>
					...
					<SortGroup
						...
						onChange={(dir) => handleChange("fechaDeIngreso", dir)}
