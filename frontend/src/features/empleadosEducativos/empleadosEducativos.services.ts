...
export const getAsignacionesEmpleadoEducativo = async (
	empleadoId: number,
): Promise<EmpleadoEducativoAsignacionesDTO> => {
	console.log("[getAsignacionesEmpleadoEducativo] empleadoId:", empleadoId);

	const { data } = await http.get<EmpleadoEducativoAsignacionesDTO>(
		`/empleadosEducativos/${empleadoId}/asignaciones`,
	);

	console.log("[getAsignacionesEmpleadoEducativo] response:", data);

	return data;
};

export const getLicenciasEmpleadoEducativo = async (
	empleadoId: number,
